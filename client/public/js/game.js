...
const { monsterAssetData, towerAssetData, gameAssetData, waveLevelAssetData } =
     getGameAssets();
...
function drawScoreboard(ctx, scoreBoardImage) {
     ...
     const height = canvasHeight * 0.25;
...
function gameLoop() {
     ...
     ctx.fillText(`현재 레벨: ${monsterLevel}`, 50, 200); // 최고 기록 표시
     ...
     towers.forEach((tower) => {
          ...
          monsters.forEach((monster) => {
               const distance = Math.sqrt(
                    Math.pow(tower.x - monster.x, 2) +
                         Math.pow(tower.y - monster.y, 2)
               );
...
Promise.all([
     ...
     ...towers.map(
          (tower) => new Promise((resolve) => (tower.image.onload = resolve))
     ),
]).then(() => {
     /* 서버 접속 코드 (여기도 완성해주세요!) */
     let somewhere;
...
function responseSellTower(data) {
     ...
     const index = towers.findIndex(
          (tower) => tower.uniqueId === sellTower.uniqueId
     );
