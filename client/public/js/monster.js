...
export class Monster {
  constructor(path, monsterImages, monsters, monsterLevel) {
    ...
    this.currentIndex = path.length - 1; // 몬스터가 이동 중인 경로의 인덱스
    this.x = path[this.currentIndex].x; // 몬스터의 x 좌표 (최초 위치는 경로의 첫 번째 지점)
    this.y = path[this.currentIndex].y; // 몬스터의 y 좌표 (최초 위치는 경로의 첫 번째 지점)
  ...
  init(monsters, level) {
    this.maxHp = monsters[this.monsterId - 1].hp * level; // 몬스터의 현재 HP
