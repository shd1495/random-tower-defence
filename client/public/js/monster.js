import { sendMonsterEvent } from "./socket";

export class Monster {
     constructor(path, monsterImages, monsters, waveLevel) {
          // 생성자 안에서 몬스터의 속성을 정의한다고 생각하시면 됩니다!
          if (!path || path.length <= 0) {
               throw new Error("몬스터가 이동할 경로가 필요합니다.");
          }

          this.monsterId = Math.floor(Math.random() * monsters.length); // 몬스터 번호 (1 ~ 5. 몬스터를 추가해도 숫자가 자동으로 매겨집니다!)
          ...
          this.level = waveLevel; // 몬스터 레벨
          this.init(monsters, level);
     }

     init(monsters, level) {
          this.maxHp = monsters[this.monsterId].hp * level; // 몬스터의 현재 HP
          this.hp = this.maxHp; // 몬스터의 현재 HP
          this.speed = 2 * level;
          this.attackPower = monsters[this.monsterId].power * level;
          this.score = monsters[this.monsterId].score * level;
          this.reward = monsters[this.monsterId].reward * level;
     }
