...
export async function signup(req, res) {
     try {
          ...
          if (existingAccount) {
               return res
                    .status(400)
                    .json({ message: "이미 사용 중인 계정 ID입니다." });
          }
          ...
          return res
               .status(201)
               .json({ message: "회원 가입에 성공", account: newAccount });
     } catch (error) {
...
export async function signin(req, res, next) {
     try {
          ...
          if (!existingAccount) {
               ...
               res.status(400).json({ message: "계정이 존재 하지 않습니다." });
          }
          ...
          return res
               .status(201)
               .json({ message: "로그인 성공", account: accountId });
     } catch (error) {
          next(error);
     }
}
