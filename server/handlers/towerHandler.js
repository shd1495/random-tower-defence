...
export const towerCreateInit = async (uuid, payload) => {
    ...
    // 클라에서 받은 데이터들
    let { uniqueId, towerId, towerCount, tower } = payload;

    // 검증 모두 성공하면 타워 생성(타워 종류 생기면 코드 수정 필요)
    await setTower(uuid, uniqueId, towerId, tower);
...
export const towerCreate = async (uuid, payload) => {
    ...
    let redisTowers = await getTowers(uuid);
    // 클라에서 받은 데이터들
    let { userGold, uniqueId, towerCount, towerId, towerType, tower } = payload;
