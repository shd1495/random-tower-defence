 ...
 * @returns
 */
export const waveLevelIncrease = (uuid, payload) => {
  // paload는 : score, currentLevel, nextLevel

  // 유저의 현재 스테이지 정보 불러오기
  const currentWave = getWaveLevel(uuid);

  // 해당 유저의 wave 정보가 없다
  if (!currentWave) {
    return { status: 'fail', message: 'No waves found for user' };
  }

  // 서버 vs 클라이언트 현재 같은 waveLevel이 맞는지 체크
  if (currentWave.id !== payload.currentLevel) {
    return { status: 'fail', message: 'Current wave mismatch' };
  }
  ...
  if (!waveLevel.data.some((wave) => wave.id === payload.nextLevel)) {
    return { status: 'fail', message: 'Next wave not found' };
  }

  // 점수 검증
  if (!scoreValidation(uuid, payload)) {
    return { status: 'fail', message: 'Invalid elapsed time' };
  }

  // 다음 웨이브 id 추가
  setWaveLevel(uuid, payload.nextLevel);
